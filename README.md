# CYPHER - Network Intrusion Detection System
...
CYPHER is a two-stage ML pipeline designed for real-time network threat detection:
...
# 4. Ensure model artifacts are in artifacts/
...
* Consider GPU support for PyTorch inference with `--gpus all`
...
| ----------------- | ---------------------------------- | ------------ |
| `CSV_FOLDER`      | Path to CICIDS CSV files           | `main.py`    |
...
The system is designed to work with **CICIDS2017**-style network flow data. The `/random-analyze` endpoint reads from CSV files in the configured `CSV_FOLDER` path.

Expected CSV format: CICIDS2017 feature columns with optional `Label` column, which is dropped during random sampling.
